...
/* 
 * Update from almprokdr@gmail.com
 * 1. Allowes pdf file
 * 2. Create jpg file form first page pdf file if defined ghostscript 
 * 3. add response upload_name when upload file
 *
 */ 
...
class UploadHandler
{
	...
	const APPLCTYPE_PDF  = 'application/pdf';
	...
	protected function initialize() {
		//	print_r($this->get_server_var('REQUEST_METHOD')); die();
		...
		if ($this->is_valid_file_object($file_name)) {
			...
			foreach ($this->options['image_versions'] as $version => $options) {
				if (!empty($version)) {
					...
					$file_name = strtolower($ext['extension'])=='pdf'?$ext['filename'].'.jpg' : $file_name;
					if (is_file($this->get_upload_path($file_name, $version))) {
						$file->{$version.'Url'} = $this->get_download_url(
							//if pdf file return jpeg image thumbnail
							$file_name,//$file->name, 
							$version
						);
					}
	...
	protected function get_valid_image_extensions($file_path) {
		switch ($this->imagetype($file_path)) {
			case self::IMAGETYPE_JPEG: return array('jpg', 'jpeg');
			case self::IMAGETYPE_PNG: return  array('png');
			case self::IMAGETYPE_GIF: return array('gif');
			case self::APPLCTYPE_PDF: return array('pdf');
		}
	}
	...
	protected function pdf_create_scaled_image($file_name, $version, $options) {
		if(!$this->options['gs_command']) return false;
		$list = $this->get_scaled_image_file_paths($file_name, $version);
		$src = $list[0]; $des = str_replace('.'.$this->is_pdf,'.jpg',$list[1]);
		$dpi = $this->options['gs_image_dpi'];
		$qty = $this->options['gs_image_quality'];
		$cmd = '-dSAFER -dBATCH -dNOPAUSE -sDEVICE=jpeg  -r'.$dpi.' -dNumRenderingThreads=4 -dFirstPage=1 -dLastPage=1 -o"'.$des.'" -dJPEGQ='.$qty.' -q "'.$src.'" -c quit';
		$out = $this->executeGS($cmd);
		return true;
	}
    ...
    private function executeGS($command, $is_shell = false){
        return $this->execute($this->options['gs_command']." ".$command, $is_shell);
    }

	
	protected function create_scaled_image($file_name, $version, $options) {
		if($this->is_pdf){ 
			return $this->pdf_create_scaled_image($file_name, $version, $options);
		}else{
			try {
				if ($this->options['image_library'] === 2) {
	...
	protected function imagetype($file_path) {
		...
		if (bin2hex(@$data[0]).substr($data, 1, 4) === '25PDF') {
			return self::APPLCTYPE_PDF;
		}
		return false;
	}
	...
	protected function has_image_file_extension($file_path) {
		return !!preg_match('/\.(gif|jpe?g|png|pdf)$/i', $file_path);
	}
	...
	protected function handle_image_file($file_path, $file) {
		...
		foreach ($this->options['image_versions'] as $version => $options) {			
			if ($this->create_scaled_image($file->name, $version, $options)) {
				if (!empty($version)) {
					$thumb = $this->get_download_url( $file->name, $version );
					// Fix response upload if pdf file
					$file->{$version.'Url'} = $this->is_pdf ? str_replace('.'.$this->is_pdf, '.jpg', $thumb) : $thumb;
				} else {
	...
	protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
		...
		$file = new \stdClass();
		$ext = pathinfo($name);
		$this->is_pdf = strtolower($ext['extension'])=='pdf' ? $ext['extension'] : false;
		$file->upload_name = $name;
			...
			if ($success) {
				foreach ($this->options['image_versions'] as $version => $options) {
					if (!empty($version)) {						
						...
						$ext = pathinfo($file);
						$file = strtolower($ext['extension'])=='pdf' ? str_replace('.'.$ext['extension'], '.jpg', $file) : $file;						
						if (is_file($file)) {
							unlink($file);
						}
					}
